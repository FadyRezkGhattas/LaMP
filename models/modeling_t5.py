...
from .parallel_utils import match_attn_matrices_for_parallel, adjust_tensors_for_parallel
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        def shape(states):
            """projection"""
            # return states.view(batch_size, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2)
            batch_dim = max(batch_size, states.shape[0])
            return states.view(batch_dim, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2)
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        (hidden_states,) = adjust_tensors_for_parallel(attention_output[0], hidden_states)
    ...
    def forward(
        ...
        for lm_logits in lm_logits_reshaped:
            if labels is not None:
                ...
                loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1)).detach().clone().item()
            losses.append(loss)
